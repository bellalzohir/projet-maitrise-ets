...
DB_CONFIG_FILE =  FAAS_ROOT+'/db_local.ini'
...
def GetActivationRecordsSince(since, limit=100):
    ...
    configs = GetDBConfigs()
    url = configs['db_protocol']+'://'+configs['db_host']+':' + \
        configs['db_port']+'/'+'test_activations/_find'
    header = {
        "Content-Type": "application/json"
    }
    body = {
    ...
    res = request('POST', url, body=json.dumps(body), headers=header,
                  auth='%s:%s' % (configs['db_username'], configs['db_password']),verbose=False)
