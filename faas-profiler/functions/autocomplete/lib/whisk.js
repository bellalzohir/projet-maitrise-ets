...
const createAction = function(name, path) {
  ...
  var createParams = ['action', 'update', 'autocomplete/'+name, '--kind', 'nodejs:14', path, '-a', 'web-export', 'true', '-i'];
...
const url = function(namespace, package, action) {
  return 'https://localhost:31001/api/v1/web/' + namespace + '/' + package + '/' + action;
}
