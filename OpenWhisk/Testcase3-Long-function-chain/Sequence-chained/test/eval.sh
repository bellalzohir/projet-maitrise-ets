...
if [[ -z `wsk -i action list | grep $ACTIONNAME` || -z `wsk -i action list | grep $SEQUENCENAME` ]];then
...
./single-cold_warm.sh -m cold -t $cold_loop -r $result -l

echo "2. measuring warm-invoke..."
./single-cold_warm.sh -t $warm_loop -w $warm_warmup -r $result -l
